...
public class ClassFileCreator {
        ...
        for (final Map.Entry<LectureForCalendar, List<String>> participantsEntries : participantsByEvent.entrySet()) {
            ...
            if (calendarEntriesByEvent.containsKey(lecture)) {
                ...
                final File classFile =
                    ClassFileCreator.computeClassFile(
                        lecture,
                        calendarEntries,
                ...
                final MetaInformation meta =
                    Main.parseMetaInformation(
                        classFile.getAbsoluteFile().toPath().getParent().resolve("meta.json").toFile()
                    );
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(classFile))) {
                    ...
                    for (final OCEntry calendarEntry : calendarEntries) {
                        writer.write(ClassFileCreator.toClassFileLine(calendarEntry));
                        writer.write("\n");
                    }
                    switch (meta.type()) {
                    case TALK40QUIZ10:
    ...
    private static File computeClassFile(
        ...
        final String folder = ClassFileCreator.folderForLecture(lecture.title());
        final String classFileName = ClassFileCreator.toClassFileName(lecture, calendarEntries);
    ...
    private static String folderForLecture(final String lecture) {
        switch (lecture) {
        case "Advanced Topics in Computer Science":
            return "AdvancedTopicsInComputerScience";
        ...
        case "Transferprojekt: Software Engineering":
        ...
        case "Mathematische Grundlagen":
            return "MathematischeGrundlagen";
        default:
    ...
    private static String toClassFileName(final LectureForCalendar lecture, final List<OCEntry> calendarEntries) {
        ...
        result.append(ClassFileCreator.toQuarter(calendarEntries));
